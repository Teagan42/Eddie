...
function defaultFormatThemeLabel(theme: string): string {
  return theme
    .split(/[_-]/)
    .filter(Boolean)
...
export interface OverviewHeroProps {
  ...
  formatThemeLabel?: (theme: string) => string;
  launchHref?: string;
  launchLabel?: string;
  LaunchLinkComponent?: ComponentType<{ href: string; children: ReactNode }>;
}
...
export function OverviewHero({
  ...
  launchHref = '/chat',
  launchLabel = 'Launch Chat Console',
  LaunchLinkComponent,
}: OverviewHeroProps): JSX.Element {
  const themeOptions = availableThemes && availableThemes.length > 0 ? availableThemes : [theme];
  const LinkComponent = LaunchLinkComponent;
      ...
      <Flex direction={{ initial: 'column', md: 'row' }} justify="between" gap="6" align="start">
        <Flex direction="column" gap="5" className="max-w-2xl">
          ...
          <Flex gap="3" wrap="wrap">
            ...
            >
              {LinkComponent ? (
                <LinkComponent href={launchHref}>{launchContent}</LinkComponent>
              ) : (
                <a href={launchHref} className="inline-flex items-center gap-2">
                  {launchContent}
                </a>
              )}
            </Button>
          ...
          <Flex direction="column" gap="4">
            ...
            {apiKey ? (
              ...
              <Button size="2" variant="soft" color="grass" asChild>
                <a href="#authentication">Add key</a>
              </Button>
            )}
          </Flex>
        </Box>
