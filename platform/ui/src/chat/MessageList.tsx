...
import type {
  ...
  ChatMessageRole,
  MessageListItem,
  ChatMessageReasoningSegment,
} from './types';

type MessageWithMetadata = MessageListItem;
...
const MESSAGE_ROLE_STYLES: Record<ChatMessageRole, MessageRoleStyle> = {
  user: {
    label: 'User',
    badgeColor: 'blue',
    align: 'end',
...
function getParentFromLineage(agent: ChatMessageMetadata['agent']): string | null {
...
function getReasoningAgentLabel(
  message: MessageWithMetadata,
  segment: ChatMessageReasoningSegment | undefined,
): string | null {
...
function getRenderableReasoningSegments(
  reasoning: MessageWithMetadata['reasoning'],
): Array<{ segment: ChatMessageReasoningSegment; text: string }> {
  if (!reasoning || !Array.isArray(reasoning.segments)) {
    return [];
  }

  const segments: Array<{ segment: ChatMessageReasoningSegment; text: string }> = [];
...
function getReasoningAgentParentLabel(message: MessageWithMetadata): string | null {
  ...
  return firstNonEmpty(
    ...
    getParentFromLineage(agentMetadata),
  );
}
