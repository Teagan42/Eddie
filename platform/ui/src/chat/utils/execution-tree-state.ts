...
export function applyToolCallEvent(
  ...
  mutableTarget.args = parsedArgs as Record<string, unknown>;
...
export function applyToolResultEvent(
  ...
  mutableTarget.result = parsedResult as Record<string, unknown>;
  target.agentId = normalizeAgentId(payload.agentId) ?? target.agentId ?? UNKNOWN_AGENT_ID;
...
function convertContextBundles(
  ...
  const converted = bundles.map((bundle) => ({
    ...
    type: 'other',
    media: '',
    createdAt: new Date().toISOString() ?? UNIX_EPOCH,
...
function cloneToolInvocations(
  ...
  return nodes.map((node) => {
    ...
    if (!descriptors.args) {
      descriptors.args = {
        value: (metadataClone as { args?: Record<string, unknown> } | undefined)?.args,
        writable: true,
        enumerable: true,
        configurable: true,
      };
    ...
    if (!descriptors.result) {
      descriptors.result = {
        value: (metadataClone as { result?: Record<string, unknown> } | undefined)?.result,
        writable: true,
        enumerable: true,
        configurable: true,
      };
...
function cloneContextBundleFiles(
  ...
  }) as ExecutionContextBundle['files'];
}
...
function normalizeInvocationNode(node: OrchestratorToolCallNodeDto): ExecutionToolInvocationNode {
  ...
  if (metadataClone && 'args' in metadataClone) {
    (normalized as MutableToolInvocationNode).args = (metadataClone as { args?: Record<string, unknown> }).args;
  }
  if (metadataClone && 'result' in metadataClone) {
    (normalized as MutableToolInvocationNode).result = (metadataClone as { result?: Record<string, unknown> }).result;
  }
