...
describe("ContextService byte budget checks", () => {
  ...
  it("skips files exceeding the byte budget without reading them", async () => {
    ...
    expect(fsStreamMocks.createReadStream).not.toHaveBeenCalled();
  ...
  it("includes files within the byte budget after stat check", async () => {
    ...
    fsStreamMocks.createReadStream.mockReturnValueOnce(
      Readable.from(["x".repeat(40)], { encoding: "utf-8" })
    );
  ...
  it("streams file contents when direct reads fail", async () => {
    const service = createService();
    globMock.mockResolvedValueOnce(["large.txt"]);
    fsMocks.stat.mockResolvedValueOnce({ size: 60 } as Stats);
    fsMocks.readFile.mockRejectedValueOnce(new Error("buffer overflow"));
    fsStreamMocks.createReadStream.mockReturnValueOnce(
      Readable.from(["x".repeat(60)], { encoding: "utf-8" })
    );

    const result = await service.pack({ baseDir: "/repo", maxBytes: 100 });

    expect(fsStreamMocks.createReadStream).toHaveBeenCalledWith(
      path.resolve("/repo", "large.txt"),
      expect.objectContaining({ encoding: "utf-8" })
    );
    expect(result.files).toHaveLength(1);
