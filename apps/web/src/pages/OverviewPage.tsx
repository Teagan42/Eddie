...
export function OverviewPage(): JSX.Element {
  ...
  const renderMessages = (messages: ChatMessageDto[]): JSX.Element => (
    <ScrollArea type="always" className="relative h-64 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] p-4">
        ...
        {messages.length === 0 ? (
          ...
          messages.map((message, index) => (
            <Flex
              key={message.id}
              direction="column"
              className="relative gap-2 rounded-2xl border border-white/10 bg-slate-950/60 p-4 shadow-[0_25px_65px_-55px_rgba(59,130,246,0.8)]"
            >
  ...
  return (
    <Flex direction="column" gap="8">
      <Box className="relative overflow-hidden rounded-[2.75rem] border border-white/10 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-10 shadow-[0_65px_120px_-60px_rgba(14,116,144,0.75)]">
        <div className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(circle_at_top_left,_rgba(96,165,250,0.22),transparent_60%),radial-gradient(circle_at_bottom_right,_rgba(34,197,94,0.22),transparent_55%)]" />
        <div className="pointer-events-none absolute inset-0 -z-10 opacity-70 blur-3xl [background:conic-gradient(from_120deg_at_50%_50%,rgba(59,130,246,0.35)_0deg,transparent_120deg,rgba(74,222,128,0.3)_240deg,transparent_360deg)]" />
        <Flex direction={{ initial: "column", md: "row" }} justify="between" gap="6" align="start">
          <Flex direction="column" gap="5" className="max-w-2xl">
            <Badge color="grass" variant="surface" radius="full" className="w-fit bg-emerald-500/10 text-emerald-100">
              Mission Control
            </Badge>
            <Heading size="8" className="text-balance text-white drop-shadow-lg">
              Operate your agentic fleet with cinematic clarity.
            </Heading>
            <Text size="3" className="text-foreground/70">
            ...
            <Flex gap="3" wrap="wrap">
              <Button
                size="3"
                variant="solid"
                className="bg-gradient-to-r from-emerald-400 via-emerald-500 to-sky-500 text-slate-950 shadow-[0_30px_70px_-40px_rgba(45,212,191,0.9)]"
                asChild
              >
          ...
          <Box className="relative w-full max-w-xs overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
            <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_top,_rgba(16,185,129,0.25),transparent_65%)]" />
        ...
        <Grid columns={{ initial: "1", sm: "3" }} gap="4">
          {stats.map((stat) => {
            ...
            return (
              <Box
                key={stat.label}
                className="group relative overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_35px_70px_-55px_rgba(56,189,248,0.85)]"
              >
      ...
      <Grid columns={{ initial: "1", xl: "2" }} gap="6">
        ...
        >
          <Grid columns={{ initial: "1", md: "2" }} gap="5">
            <ScrollArea type="always" className="h-80 rounded-2xl border border-white/10 bg-slate-950/40 p-3">
                ...
                {sessionsQuery.data?.length ? (
                  sessionsQuery.data.map((session) => (
                    <button
                      ...
                      className={cn(
                        "rounded-2xl border border-white/5 px-4 py-3 text-left transition-all",
                        session.id === selectedSessionId
                          ? "bg-emerald-500/20 text-white shadow-[0_18px_45px_-28px_rgba(16,185,129,0.8)]"
                          : "bg-white/[0.02] text-foreground/80 hover:-translate-y-0.5 hover:border-emerald-500/40 hover:bg-emerald-500/10"
                      )}
                    >
              ...
              <form onSubmit={handleSendMessage} className="flex flex-col gap-3">
                ...
                <Flex gap="2" justify="end">
                  <Button
                    ...
                    size="3"
                    disabled={!selectedSessionId || createMessageMutation.isPending}
                    className="bg-gradient-to-r from-emerald-400 via-emerald-500 to-sky-500 text-slate-950"
                  >
                    <PaperPlaneIcon /> Send
                  </Button>
                </Flex>
        ...
        <Panel title="Traces" description="Real-time observability into orchestrated workloads">
          {tracesQuery.isLoading ? (
            ...
            <ScrollArea type="always" className="h-80 rounded-2xl border border-white/10 bg-slate-950/40 p-4">
      ...
      <Grid columns={{ initial: "1", xl: "2" }} gap="6">
        ...
        >
          {configQuery.isLoading ? (
            ...
            <Flex direction="column" gap="4">
              <Box className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                <Text size="1" color="gray">
                  API URL
                </Text>
