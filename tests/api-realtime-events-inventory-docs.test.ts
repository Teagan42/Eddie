...
const requiredTableRows = Object.freeze([
  rowPattern("ChatSessionsService", "session.created"),
  rowPattern("ChatSessionsService", "ChatMessageCreatedEvent"),
  rowPattern("ChatSessionStreamRendererService", "ChatMessagePartialEvent"),
  rowPattern("TracesService", "trace.created"),
  rowPattern("RuntimeConfigService", "config.updated"),
  rowPattern("LogsForwarderService", "logs.created"),
]);

function rowPattern(source: string, event: string): RegExp {
  return new RegExp(
    `\\| ${escapeRegex(source)} \\| ${escapeRegex(event)} \\|`
  );
}
